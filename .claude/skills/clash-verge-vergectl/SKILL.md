...
Use `vergectl` instead of editing Clash Verge config files directly. It talks to the running app, so config changes go through the same backend side effects as the UI.

Before answering anything about profile content or config format, read **Profile Format** below. Guessing the format from a file extension is the single most common failure mode here.
...
A main profile (`local` / `remote`) has **two possible formats**, and the extension of the `file` field does NOT tell you which one is live.

**Determine the format from `vergectl profiles list --json`, using these fields only:**

| Field | Meaning |
|---|---|
| `effective_file` | The file actually read, enhanced, edited and validated. **This is the source of truth.** |
| `profile_format` | `"yaml"` or `"conf"`. |
| `conf_override` | `true` means a `.conf` sibling exists and fully overrides the YAML. |
| `file` | The **declared** file, always `.yaml`. Says nothing about the live format. Never infer from it. |
| `name` | A display label. May end in `.yaml` regardless of format. Never infer from it. |

When `conf_override: true`, the profile is **Surge-style CONF**, not YAML. Any snippet you propose must be CONF syntax:

```
[Proxy]
NodeName = vless, 1.2.3.4, 443, uuid=..., network=tcp, tls=true, udp=true, flow=xtls-rprx-vision, servername=example.com, client-fingerprint=chrome, reality-opts={"public-key":"...","short-id":"..."}

[Proxy Group]
manual = select, DIRECT, NodeName
```

Sections: `[General]` `[Proxy]` `[Proxy Group]` `[Proxy Provider]` `[Rule Provider]` `[Rule]` `[Host]` `[Mihomo]`.

**YAML cannot be removed from this project.** It is not "the other format" — it is the internal representation and the only thing the core accepts:

- Both YAML and CONF parse into the same `serde_yaml::Mapping`; the runtime file handed to Mihomo is always YAML.
- CONF is an optional override. When no `.conf` sibling exists, the YAML declaration file is used.
- Remote subscription updates always write the declared YAML and never overwrite CONF.
- Merge / Rules / Proxies / Groups / global extensions do **not** support CONF siblings — YAML only.

Full CONF grammar, conversion rules and file lifecycle: `spec/src-tauri/src/config/profile_format.md`.

To inspect what the core actually received (always YAML, regardless of profile format):
...
This repo mirrors its source tree under `spec/`. Read the spec for a file **before** reasoning about that file — `src-tauri/src/config/profile_format.rs` → `spec/src-tauri/src/config/profile_format.md`. See root `CLAUDE.md` for the full convention.

Relevant entry points:

- `spec/src-tauri/src/config/profile_format.md` — YAML/CONF formats, conversion, lifecycle
- `spec/src-tauri/src/module/external_provider.md` — `verge-format: surge` external providers
- `spec/crates/vergectl/README.md` — vergectl itself
- `spec/README.md` — product capability inventory, to avoid rebuilding flows the UI already has

## Output Parsing

`vergectl --json` emits **indented, pretty-printed** JSON (`"name": "value"`, space after the colon). Compact-JSON grep patterns like `'"name":"..."'` silently match nothing. Prefer `jq`, and never treat an empty grep result as proof that something does not exist.
